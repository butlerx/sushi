...
func newIssue(g *gocui.Gui, v *gocui.View) error {
	previousView = v
	maxX, maxY := g.Size()
	if issueprompt, err := g.SetView("issueprompt", maxX/4, maxY/6, maxX-(maxX/4), maxY/3); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		fmt.Fprintln(issueprompt, "Please enter issue title\n(Title is mandatory)\n\nCtrl + c to cancel")
	}
	if issueEd, err := g.SetView("issueEd", maxX/4, maxY/3, maxX-(maxX/4), maxY-(maxY/6)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func cancel(g *gocui.Gui, v *gocui.View) error {
	...
	} else if (g.CurrentView()).Name() == "selectionPane" {
		if err := g.DeleteView("issueprompt"); err != nil {
			return err
		}
		if err := g.DeleteView("selectionPane"); err != nil {
